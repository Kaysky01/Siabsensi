...
<section>
  <div class="page-header">
    <div>
      <div class="page-title">Absen Sesi: {{ $sesi->nama_sesi }}</div>
      <div class="page-sub">
    ...
    <form method="POST" action="{{ route('garda.absensi-manual.store', $sesi->id) }}" id="absensi-form">
      ...
      <div style="margin-bottom:16px">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <input type="text" id="search-box" class="form-input" style="flex:1;min-width:200px" placeholder="Cari nama, NIM, prodi..." value="{{ $search }}" onkeydown="if(event.key === 'Enter'){ event.preventDefault(); applySearch(); }">
      ...
      <div style="margin-top:24px;padding-top:24px;border-top:2px solid var(--border-color);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px">
        <div>
          <div style="font-size:15px;font-weight:700;color:var(--text-main);margin-bottom:4px">
            Total Mahasiswa Keseluruhan (Kompi): <span style="color:#4f46e5">{{ $totalMahasiswaCount }}</span> Mahasiswa
          </div>
          <div style="font-size:13px;color:var(--text-muted);margin-bottom:4px">
            Summary Seluruh Data Kompi: 
  ...
  <div class="panel" style="margin-top:24px;background:#eef2ff;border-left:4px solid #4f46e5">
    <div style="font-size:14px;line-height:1.8;color:#3730a3">
      <strong>💡 Informasi Sistem Absensi Sesi Garda:</strong>
      <ul style="margin:8px 0 0 20px">
        <li><strong>Simpan Absensi Sesi</strong> akan menyimpan/memperbarui data absensi untuk <strong>SELURUH {{ $totalMahasiswaCount }} MAHASISWA KOMPI</strong> di sesi ini.</li>
        <li>Aksi massal (<em>"Semua Hadir" / "Semua Alpha"</em>) akan diterapkan ke <strong>seluruh data mahasiswa kompi</strong> di sesi ini.</li>
...
function setAllStatus(targetStatus) {
  ...
  Swal.fire({
    ...
    position: 'top-end',
    icon: 'info',
    title: `Aksi Massal: "Semua ${statusNames[targetStatus]}" akan diterapkan ke SELURUH {{ $totalMahasiswaCount }} mahasiswa kompi saat disimpan.`,
...
function confirmSave() {
  ...
  if (bulkAction) {
    const statusNames = { 'present': 'Hadir', 'alpha': 'Alpha', 'izin': 'Izin', 'sakit': 'Sakit' };
    bulkNotice = `<p style="color:#4f46e5;font-weight:600;margin-top:8px">ℹ️ Aksi Massal Aktif: <strong>Semua ${statusNames[bulkAction]}</strong> akan diterapkan ke seluruh ${totalMahasiswa} mahasiswa kompi (kecuali yang belum absen masuk yang otomatis Alpha).</p>`;
  }
  ...
  Swal.fire({
    ...
    html: `<div style="text-align:left">
      <p>Anda akan menyimpan absensi sesi untuk <strong>SELURUH ${totalMahasiswa} MAHASISWA KOMPI</strong>.</p>
      ${bulkNotice}
