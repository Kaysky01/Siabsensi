...
<section>
  <div class="page-header">
    <div>
      ...
      <div class="page-sub">Kelola pengajuan izin dan sakit dari mahasiswa kompi Anda</div>
    </div>
  </div>
  ...
  <div class="stats-row" style="display:flex;gap:16px;margin-bottom:24px">
    <a href="{{ route('garda.izin') }}" class="stat-box {{ !$filterStatus ? 'active' : '' }}">
      <div class="stat-label">Semua</div>
      <div class="stat-value">{{ $stats['pending'] + $stats['approved'] + $stats['rejected'] }}</div>
    </a>
    <a href="{{ route('garda.izin', ['status' => 'pending']) }}" class="stat-box pending {{ $filterStatus == 'pending' ? 'active' : '' }}">
      <div class="stat-label">Pending</div>
      <div class="stat-value">{{ $stats['pending'] }}</div>
    </a>
    <a href="{{ route('garda.izin', ['status' => 'approved']) }}" class="stat-box success {{ $filterStatus == 'approved' ? 'active' : '' }}">
      <div class="stat-label">Disetujui</div>
      <div class="stat-value">{{ $stats['approved'] }}</div>
    </a>
    <a href="{{ route('garda.izin', ['status' => 'rejected']) }}" class="stat-box danger {{ $filterStatus == 'rejected' ? 'active' : '' }}">
    ...
    <table class="att-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            @if($item->file_path)
              <a href="{{ asset('storage/'.$item->file_path) }}" target="_blank" class="btn btn-ghost btn-sm">Lihat</a>
            @else
          ...
          <td>
            ...
            <div style="display:flex;gap:8px">
              <button type="button" class="btn btn-primary btn-sm" onclick="approveIzin({{ $item->id }})">Setujui</button>
              <button type="button" class="btn btn-danger btn-sm" onclick="rejectIzin({{ $item->id }})">Tolak</button>
            </div>
            @else
...
function approveIzin(id) {
  Swal.fire({
    ...
    text: 'Pengajuan akan disetujui dan status kehadiran akan diperbarui.',
    icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      form.method = 'POST';
      form.action = '{{ route('garda.izin.verify') }}';
      form.innerHTML = `
