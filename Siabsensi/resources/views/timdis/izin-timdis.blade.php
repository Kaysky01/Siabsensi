...
<section>
  <div class="page-header" style="margin-bottom:20px">
    <div>
      ...
      <div class="page-sub">Kelola pengajuan izin dan sakit mahasiswa</div>
    </div>
  </div>
  ...
  <div class="panel" style="margin-bottom:16px;padding:14px 20px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px">
    ...
    <form method="GET" action="{{ route('timdis.izin-timdis') }}" style="display:flex;gap:8px;max-width:300px;width:100%">
  ...
  <div class="panel izin-desktop">
    <table class="att-table">
      <thead><tr><th style="width:50px;text-align:center">No</th><th>Mahasiswa</th><th>Kompi</th><th>Tipe</th><th>Tanggal</th><th>Alasan</th><th>Bukti</th><th>Status</th><th>Aksi</th></tr></thead>
      <tbody>
        ...
        @empty
        <tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:30px">Tidak ada pengajuan</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div>
          ...
          <div style="display:flex;gap:4px">
            ...
            <span class="badge {{ $s->submission_type === 'izin' ? 'badge-blue' : 'badge-yellow' }}">{{ strtoupper($s->submission_type) }}</span>
          </div>
        </div>
        @php $sc = match($s->status) { 'pending' => 'badge-warning', 'approved' => 'badge-green', 'rejected' => 'badge-red', default => 'badge-gray' }; @endphp
...
<div class="modal-backdrop" id="modal-verify">
  <div class="modal" style="max-width:400px;padding:24px">
    ...
    <form id="form-verify" method="POST" action="{{ route('timdis.izin.verify') }}">
      @csrf
...
<script>
  ...
  function confirmVerify(id, action) {
    ...
    if (action === 'approve') {
      ...
      desc.textContent = 'Mahasiswa akan dianggap hadir pada tanggal tersebut.';
      ...
      desc.textContent = 'Pengajuan akan ditolak. Anda bisa memberikan alasan di bawah ini.';
