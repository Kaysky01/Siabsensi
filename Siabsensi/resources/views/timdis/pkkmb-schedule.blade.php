...
<section>
  ...
  <div class="panel" style="margin-bottom:24px">
    ...
    <form method="POST" action="{{ route('timdis.pkkmb-schedule.gracePeriod') }}" style="display:flex;gap:16px;align-items:end;flex-wrap:wrap">
      @csrf
    ...
    <div class="schedule-grid">
      ...
      <div class="schedule-card {{ $schedule->is_active ? 'active' : 'inactive' }}">
        ...
        <div class="schedule-actions">
          <form method="POST" action="{{ route('timdis.pkkmb-schedule.toggle', $schedule->id) }}" style="margin:0">
            @csrf
          ...
          <form method="POST" action="{{ route('timdis.pkkmb-schedule.destroy', $schedule->id) }}" onsubmit="return confirm('Hapus jadwal Hari ke-{{ $schedule->hari_ke }}?\n\nSemua data absensi terkait akan tetap tersimpan.')" style="margin:0">
            @csrf @method('DELETE')
...
<div class="modal-backdrop" id="modal-add-schedule">
  <div class="modal modal-large">
    ...
    <form method="POST" action="{{ route('timdis.pkkmb-schedule.store') }}">
      @csrf
...
function openEditSchedule(id, hariKe, tanggal, checkInStart, checkInEnd, checkOutStart, checkOutEnd, isActive) {
  document.getElementById('edit-schedule-form').action = '/timdis/pkkmb-schedule/' + id;
