...
@section('title', 'Dashboard Tim Disiplin — SIABSEN')

@section('content')
<div class="timdis-dashboard-wrapper">
  <!-- Welcome Hero Banner -->
  <div class="timdis-welcome-banner">
    <div class="banner-content">
      <div class="banner-badge">
        <span class="pulse-dot"></span> TIM DISIPLIN PKKMB {{ date('Y') }}
      </div>
      <h1 class="banner-title">Selamat Datang, {{ Auth::user()->full_name }}!</h1>
      <p class="banner-sub">
        <span class="material-symbols-outlined icon-sm">calendar_today</span> {{ Carbon\Carbon::today()->translatedFormat('l, d F Y') }} • @if($assignedKompi) Scope: <span class="kompi-tag">Kompi {{ $assignedKompi }}</span> @else Scope: <span class="kompi-tag">Seluruh Kompi</span> @endif
      </p>
    </div>
    <div class="banner-actions">
      <a href="{{ route('timdis.attendance') }}" class="btn btn-banner-primary">
        <span class="material-symbols-outlined">sensors</span> Monitor Live
      </a>
      <a href="{{ route('timdis.kehadiran-timdis') }}" class="btn btn-banner-secondary">
        <span class="material-symbols-outlined">how_to_reg</span> Verifikasi
      </a>
      <a href="{{ route('timdis.dashboard') }}" class="btn btn-banner-ghost" title="Refresh Data">
        <span class="material-symbols-outlined">refresh</span>
      </a>
    </div>
  </div>

  <!-- Pending Alert Box (If Pending Submissions Exist) -->
  @if(isset($totalPending) && $totalPending > 0)
    <div class="pending-alert-box">
      ...
      <div class="alert-info">
        <div class="alert-title">Pengajuan Disiplin Membutuhkan Verifikasi</div>
        <div class="alert-desc">
          Terdapat <strong>{{ $totalPending }} pengajuan</strong> ({{ $pendingKehadiranCount }} Presensi Manual & {{ $pendingIzinCount }} Izin/Sakit) menunggu tindakan Timdis.
        </div>
      </div>
      <div class="alert-actions">
        @if($pendingKehadiranCount > 0)
          <a href="{{ route('timdis.kehadiran-timdis') }}" class="btn btn-alert-action">Presensi Manual ({{ $pendingKehadiranCount }})</a>
        @endif
        @if($pendingIzinCount > 0)
          <a href="{{ route('timdis.izin-timdis') }}" class="btn btn-alert-ghost">Izin/Sakit ({{ $pendingIzinCount }})</a>
        @endif
      </div>
    </div>
  @endif
  ...
  <!-- Stats Grid -->
  <div class="timdis-stats-grid">
    <!-- Stat 1: Total Mahasiswa -->
    <div class="timdis-stat-card card-blue">
      <div class="card-top">
        <div class="card-icon">
      ...
      <div class="card-desc">@if($assignedKompi) Kompi {{ $assignedKompi }} @else Seluruh Kompi @endif</div>
    </div>

    <!-- Stat 2: Hadir Hari Ini -->
    <div class="timdis-stat-card card-emerald">
      <div class="card-top">
        <div class="card-icon">
    ...
    <div class="timdis-stat-card card-rose">
      <div class="card-top">
        <div class="card-icon">
      ...
      <div class="card-desc">Belum melakukan check-in</div>
    </div>

    <!-- Stat 4: Masih Di Lokasi -->
    <div class="timdis-stat-card card-amber">
      <div class="card-top">
        <div class="card-icon">
  ...
  <!-- Main Grid Layout -->
  <div class="timdis-main-grid">
    <!-- Left Column: Absensi Terkini -->
    <div class="timdis-panel main-panel">
      <div class="panel-header">
        <div>
          ...
          <div class="panel-sub">Presensi mahasiswa terbaru yang masuk ke sistem</div>
        </div>
        <a href="{{ route('timdis.attendance') }}" class="btn btn-panel-link">
          Monitor Live <span class="material-symbols-outlined icon-sm">arrow_forward</span>
        </a>
      </div>

      <div class="table-responsive">
        <table class="timdis-table">
          <thead>
            <tr>
              <th>Mahasiswa</th>
              <th>Kompi</th>
            ...
            @empty
              <tr>
                <td colspan="4" class="table-empty-state">
                  ...
                  <div>Belum ada data presensi tercatat hari ini</div>
                </td>
              </tr>
            @endforelse
          </tbody>
    ...
    <!-- Right Side Column -->
    <div class="timdis-side-column">
      <!-- 7-Day Trend Chart -->
      <div class="timdis-panel">
      ...
      <!-- Per Kompi Distribution -->
      <div class="timdis-panel">
        <div class="panel-header-simple">
          <span class="material-symbols-outlined">pie_chart</span>
          Distribusi Presensi Per Kompi
        </div>
      ...
      <div class="timdis-panel">
        ...
        <div class="timdis-quick-grid">
          <a href="{{ route('timdis.mahasiswa') }}" class="timdis-quick-card">
            <div class="quick-card-icon bg-light-blue">
              <span class="material-symbols-outlined">school</span>
            </div>
            <div>
              <div class="quick-card-title">Data Mahasiswa</div>
              <div class="quick-card-sub">Daftar & Edit</div>
            </div>
          </a>

          <a href="{{ route('timdis.kehadiran-timdis') }}" class="timdis-quick-card">
          ...
          <a href="{{ route('timdis.izin-timdis') }}" class="timdis-quick-card">
            <div class="quick-card-icon bg-light-amber">
              <span class="material-symbols-outlined">edit_note</span>
            </div>
            <div>
              <div class="quick-card-title">Verifikasi Izin</div>
              <div class="quick-card-sub">Izin / Sakit</div>
            </div>
          </a>

          <a href="{{ route('timdis.monitoring-kegiatan') }}" class="timdis-quick-card">
            <div class="quick-card-icon bg-light-indigo">
              <span class="material-symbols-outlined">monitoring</span>
            </div>
            <div>
              <div class="quick-card-title">Monitor Sesi</div>
              <div class="quick-card-sub">Laporan kegiatan</div>
            </div>
          </a>
        </div>
      </div>
...
<style>
/* CSS Styles for Timdis Dashboard */
.timdis-dashboard-wrapper {
  display: flex;
